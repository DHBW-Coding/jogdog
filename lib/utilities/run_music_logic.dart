...
class SensorData {
  ...
  final List<double> _speeds =
      []; // List is a pointer pointing to different doubles thats why final
  ...
  SensorData._internal() {
    ...
    Timer.periodic(const Duration(seconds: sec), (timer) {
      if (!isRunning) {
        ...
        _streamCtrl.add(median(
            _speeds)); // TEST: If data is not good enough make something better
  ...
  bool _isDataReliable(List<double> speeds) {
    ...
    for (int i = 1; i < 4; i++) {
      if (speeds[lenght - i] == 0.0 ||
          speeds[lenght - i] >
              0.5) // If the Data is exactly 0.0 then there is no GPS Data
      {
        return false;
      }
    }
